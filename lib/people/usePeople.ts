...
export function useResolveAuthors(slugs: string[] | null | undefined): {
  ...
  const authors = useMemo(() => {
    ...
    return slugs.map((slug) => people[slug] || createFallbackPerson(slug)).filter((person): person is Person => person !== null);
  }, [slugs, people]);
...
export function useSearchPeople(query: string): {
  ...
  const results = useMemo(() => {
    ...
    return peopleList.filter((person) => person.name.toLowerCase().includes(queryLower) || person.slug.includes(queryLower));
  }, [query, peopleList]);
